...
import { 
  ...
  SlimContextError, 
  MCPToolResult} from '../types/index.js';

export async function handleTrimMessages(args: unknown): Promise<MCPToolResult> {
  try {
    ...
    const compressedMessages = await trimCompressor.compress(validatedArgs.messages.map(msg => ({
      ...msg,
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            success: false,
            error: errorMessage,
            error_type: error instanceof SlimContextError ? 'SlimContextError' : 'UnknownError'
          }, null, 2)
        }
      ]
    };
