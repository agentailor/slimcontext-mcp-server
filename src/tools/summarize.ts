  ...
  async invoke(messages: SlimContextMessage[]) {
    try {
      const openaiMessages = messages.map(msg => ({
        role: msg.role === 'human' ? 'user' as const : 
              msg.role === 'tool' ? 'assistant' as const :
...
export async function handleSummarizeMessages(args: unknown): Promise<MCPToolResult> {
  try {
    ...
    if (!apiKey) {
      throw new SlimContextError('OpenAI API key is required. Provide it via openaiApiKey parameter or OPENAI_API_KEY environment variable.');
    }
    ...
    const compressedMessages = await summarizeCompressor.compress(validatedArgs.messages.map(msg => ({
      ...msg,
    ...
    const hasSummary = compressedMessages.some(msg => 
      msg.role === 'system' && 
    ...
    const errorType = error instanceof OpenAIError 
      ? 'OpenAIError' 
